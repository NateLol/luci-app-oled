	...
	while(1) {
		...
		if((last_rx_bytes == 0 && last_tx_bytes == 0) ||
		   ...
		   (uptime <= last_uptime)) {
			...
			rx_speed = (rx_bytes - last_rx_bytes) / (uptime - last_uptime);
			tx_speed = (tx_bytes - last_tx_bytes) / (uptime - last_uptime);
    ...
    while(1){
        ...
        for(int i = 1; i < time; i++){
			...
			pthread_mutex_unlock(&__mutex_shared_variable);
	  	        ...
	  	        testnetspeed(SPLIT, 0, rx_speed, tx_speed);
		}
                Display();
                usleep(1000000);
                clearDisplay();
			...
			testnetspeed(MERGE, 16*(date+lanip+cpufreq+cputemp+netspeed-1), rx_speed, tx_speed);
		    }
                Display();
                usleep(1000000);
                clearDisplay();
			...
			testnetspeed(FULL, 8*(date+lanip+1+netspeed-1), rx_speed, tx_speed);
		    }
                }else{
			...
			testnetspeed(FULL, 8*(date+lanip+cpufreq+cputemp+netspeed-1), rx_speed, tx_speed);
		    }
                }
                Display();
                usleep(1000000);
